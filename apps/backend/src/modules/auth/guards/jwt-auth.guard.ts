    ...
    try {
      ...
      if (authHeader) {
        ...
        if (authHeader.startsWith('Bearer Bearer ')) {
          this.logger.warn(
            'WARNING - Duplicate "Bearer" detected in Authorization header!',
          );
          this.logger.warn(
            'This usually means you included "Bearer" in Swagger UI token field.',
          );
          this.logger.warn(
            'Swagger UI automatically adds "Bearer" prefix, so just enter your token without it.',
          );
        }
        ...
        this.logger.debug(
          `Token format correct (starts with Bearer): ${authHeader.startsWith('Bearer ')}`,
        );
